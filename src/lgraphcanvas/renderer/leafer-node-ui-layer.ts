...
} from "./leafer-components/registry.ts";
import {
...
function createCompatGroup(name = "Group"): LeaferUiLike {
    return {
        __tag: name,
        children: [],
        add(...children) {
...
const clearChildren: ClearChildrenFn = (group) => {
    if (!group) {
        return;
    }
...
const addChildren: AddChildrenFn = (parent, ...children) => {
...
function removeChild(parent: LeaferUiLike | null | undefined, child: LeaferUiLike | null | undefined): void {
    if (!parent || !child) {
        return;
    }
...
function getTitleText(node: LeaferNodeLike): string {
    try {
...
function getNodeTooltipText(node: LeaferNodeLike): string {
...
export class LeaferNodeUiLayer {
    ...
    _createUi(name: string, data: Record<string, unknown> = {}): LeaferUiLike {
    ...
    _createLeaferSurface(width: number, height: number): void {
    ...
    init({
        ...
        runtime: LeaferRuntimeLike | null;
    ...
    renderTo(targetCtx: RenderContextLike | null | undefined): void {
    ...
    _runCallbackLayer(
        canvasLayer: (LeaferUiLike & { context?: RenderContextLike | null; width?: number; height?: number; paint?: () => void }) | null,
        callback: ((...args: unknown[]) => unknown) | null | undefined,
        scope: unknown,
        args: unknown[],
        scale: number,
        titleHeight: number,
    ...
    _getOrCreateView(key: unknown): NodeViewRef {
    ...
    _updateNodeVisuals(
        ...
        host: LeaferHostLike,
        view: NodeViewRef,
        hashes: ReturnType<LeaferNodeUiLayer["_computeHashes"]>,
    ): boolean {
    ...
    upsertNode(node: LeaferNodeLike, host: LeaferHostLike): boolean {
