import { scaledFontSize, snapPixel } from "./text-layout.ts";
import type { LeaferNodeComponentEnv, LeaferNodeLike, LeaferUiFactory, LiteGraphLike } from "../leafer-types.ts";

function getTitleModeColor(node: LeaferNodeLike, LiteGraph: LiteGraphLike): string | false {
    let modeColor: string | false = false;
 ...
 * @param {object} env
 */
/** 中文说明：renderNodeTitle 为迁移后的 TS 导出函数，负责 Leafer 渲染流程中的对应步骤。 */
export function renderNodeTitle(env: LeaferNodeComponentEnv): void {
    const {
        view,
        createUi,
        addChildren,
