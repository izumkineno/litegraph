...
export function renderWidgetText(env: LeaferWidgetComponentEnv): LeaferWidgetRenderResult {
    ...
    if (showText) {
        ...
        const labelText = createLeaferText(createUi, {
            x: margin * 2,
            y: sy,
            width: Math.max(1, widgetWidth * 0.45),
            height: sh,
            text: String(label),
        ...
        const valueText = createLeaferText(createUi, {
            x: margin * 2,
            y: sy,
            width: Math.max(1, widgetWidth - margin * 4),
            height: sh,
            text: value,
