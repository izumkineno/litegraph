import { scaledFontSize, snapPixel, widgetTextTop } from "./text-layout.ts";
import type { LeaferNodeComponentEnv } from "../leafer-types.ts";

/**
 * @param {object} env
 */
/** 中文说明：renderNodeTooltip 为迁移后的 TS 导出函数，负责 Leafer 渲染流程中的对应步骤。 */
export function renderNodeTooltip(env: LeaferNodeComponentEnv): void {
    const {
        view,
        createUi,
        addChildren,
