...
function resolveSlotLabelPlacement(text, slot, isOutput, sx, sy, horizontal, scale, fontSize, LiteGraph) {
...
function createGridShape(createUi, sx, sy, scale, fill) {
...
function createInputArrowShape(createUi, sx, sy, horizontal, scale, fill) {
    if (horizontal) {
...
function createOutputArrowShape(createUi, sx, sy, horizontal, scale, fill) {
    if (horizontal) {
        return createUi("Polygon", {
            points: [sx - 8 * scale, sy, sx + 4 * scale, sy + 6 * scale, sx + 4 * scale, sy - 6 * scale],
            fill,
...
function createBoxShape(createUi, sx, sy, horizontal, scale, fill) {
...
function createCircleShape(createUi, sx, sy, scale, fill, lowQuality) {
...
function createSlotGraphic(createUi, slotShape, sx, sy, horizontal, scale, fill, lowQuality, isOutput, LiteGraph) {
...
function renderCollapsedSlotIndicators(env) {
    const {
        view,
        createUi,
        addChildren,
    ...
    if (Array.isArray(node?.inputs)) {
        for (let i = 0; i < node.inputs.length; i += 1) {
            ...
            const fill = slot?.link != null
                ? (slot?.color_on || host?.default_connection_color_byType?.[slotType] || host?.default_connection_color?.input_on || tokens.slotInputColor)
                : (slot?.color_off || host?.default_connection_color_byTypeOff?.[slotType] || host?.default_connection_color_byType?.[slotType] || host?.default_connection_color?.input_off || tokens.slotInputColor);
    ...
    if (Array.isArray(node?.outputs)) {
        for (let i = 0; i < node.outputs.length; i += 1) {
            ...
            const compatible = host?.connecting_input
                ...
                : true;
            const fill = slot?.links?.length
                ? (slot?.color_on || host?.default_connection_color_byType?.[slotType] || host?.default_connection_color?.output_on || tokens.slotOutputColor)
                : (slot?.color_off || host?.default_connection_color_byTypeOff?.[slotType] || host?.default_connection_color_byType?.[slotType] || host?.default_connection_color?.output_off || tokens.slotOutputColor);
