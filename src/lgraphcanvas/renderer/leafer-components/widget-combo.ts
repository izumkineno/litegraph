...
import type { LeaferNodeLike, LeaferWidgetComponentEnv, LeaferWidgetLike, LeaferWidgetRenderResult } from "../leafer-types.ts";

function resolveComboValue(widget: LeaferWidgetLike, node: LeaferNodeLike) {
    ...
    if (values && values.constructor !== Array) {
        const resolved = values[String(value ?? "")];
        if (resolved != null) {
            value = resolved;
        }
    }
...
export function renderWidgetCombo(env: LeaferWidgetComponentEnv): LeaferWidgetRenderResult {
