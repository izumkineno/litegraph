# 文件文档：`src/lgraphcanvas/renderer/leafer-components/text-layout.js`
...
- 类型：`export_all`
- 位置：`src/lgraphcanvas/renderer/leafer-components/text-layout.js:2-2` (`#L2`)
- 转发来源：`./text-layout.ts`
- 目标文件：`src/lgraphcanvas/renderer/leafer-components/text-layout.ts` (未定位)
- 可解析导出数量：5
- 说明来源：源码注释
- 说明：
